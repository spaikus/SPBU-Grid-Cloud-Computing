...
def sha256(str: str) -> str:
    ...
    def rcshift(num32, i):
        return num32 >> i | num32 << (32-i)

    def main_loop(h, str, chunks):
        w = np.zeros(64, dtype=np.uint32)
        ind = 0
        for _ in range(chunks):
            for i in range(16):
                tmp = np.array([str[ind], str[ind+1], str[ind+2], str[ind+3]], dtype=np.uint32)
                w[i] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3]
                ind += 4
            for i in range(16, 64):
                s0 = (w[i-15] >> 3) ^ rcshift(w[i-15], 7) ^ rcshift(w[i-15], 18)
                s1 = (w[i-2] >> 10) ^ rcshift(w[i-2], 17) ^ rcshift(w[i-2], 19)
                w[i] = w[i-16] + s0 + w[i-7] + s1
            
            A = np.array([h[0]], dtype=np.uint32); B = np.array([h[1]], dtype=np.uint32) 
            C = np.array([h[2]], dtype=np.uint32); D = np.array([h[3]], dtype=np.uint32) 
            E = np.array([h[4]], dtype=np.uint32); F = np.array([h[5]], dtype=np.uint32) 
            G = np.array([h[6]], dtype=np.uint32); H = np.array([h[7]], dtype=np.uint32)

            for i in range(64):
                t1 = K[i] + w[i] + H + ((E & F) ^ ((~E) & G)) + (rcshift(E, 6) ^ rcshift(E, 11) ^ rcshift(E, 25))
                t0 = t1 + ((A & B) ^ (A & C) ^ (B & C)) + (rcshift(A, 2) ^ rcshift(A, 13) ^ rcshift(A, 22))
    ...
    bstr = np.frombuffer(str.encode(), np.uint8)
    siz = len(bstr)
    chunks = siz >> 6
    extra_siz = siz - (chunks << 6)
    extra_chunks = (extra_siz + 72) >> 6
    extra_mem = extra_chunks << 6

    extra = np.zeros(1024, dtype=np.uint8)
    extra[:extra_siz] = bstr[chunks << 6 : (chunks << 6) + extra_siz]
    extra[extra_siz] = 1 << 7
    extra[extra_siz + 1:] = 0
    siz <<= 3
    for i in range(8):
        extra[extra_mem - 1 - i] = siz >> (i << 3)
    siz >>= 3
    ...
    main_loop(h, bstr, chunks)
    main_loop(h, extra, extra_chunks)
...
def streebog256(str: str) -> str:
    ...
    def LPS(vec):
        tmp = np.zeros(64, dtype=np.uint8)
    ...
    def add(dst, src):
        ...
        for i in range(63, -1, -1):
            r += np.uint16(src[i]) + np.uint16(dst[i])
            dst[i] = r
            r >>= 8
    ...
    N = np.zeros(64, dtype=np.uint8)
    Sigma = np.zeros(64, dtype=np.uint8)

    siz = len(str)
    bstr = str.encode()
    while siz >= 64:
        siz -= 64
        m = np.frombuffer(bstr[siz : siz + 64], dtype=np.uint8)
        gN(h, m, N)
        add_u16(N, 512)
        add(Sigma, m)
    ...
    m[shift] = 1
    m[shift + 1 : shift + 1 + siz] = np.frombuffer(bstr[:siz], dtype=np.uint8)
